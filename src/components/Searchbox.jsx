...
const CollectPage = ({session, fetchedUser}) => {
    ...
    try {
      ...
      if (updatedTask) {
        setTasks(
          tasks.map((task) =>
            task.id === taskId
              ? { ...task, status: newStatus, collectorId: session?.user.id }
              : task
          )
        );
...
If a task is provided, compare the results with:
- Expected waste type: ${selectedTask.wasteType}
- Expected Quantity Matches: ${selectedTask.amount} (ensure the numerical value match, allowing for a some small tolerance range).
  ...
  "quantityMatch": true/false
      ...
      try {
        ...
        if (
          ...
          console.log(saveCollectedWaste)
          console.log("Hii")
