...
import { getUserByEmail, getWasteCollectionTasks, updateTaskStatus } from '@/actions/dbActions'
...
const CollectPage = () => {
    ...
    const handleStatusChange = async (taskId, newStatus) => {
      ...
      try {
        ...
        const updatedTask = await updateTaskStatus(taskId, newStatus, session?.user.id)  

        console.log(updatedTask)              ///////////////////////
        if (updatedTask) {
          setTasks(tasks.map(task => 
            task.id === taskId ? { ...task, status: newStatus, collectorId: session?.user.id } : task
          ))
